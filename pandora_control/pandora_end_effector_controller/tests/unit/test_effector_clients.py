...
class TestLinearClient(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.client_instance = LinearClient()
    self.effector_goal = MoveEndEffectorGoal()
    self.effector_goal.command = 'TEST'
    self.effector_goal.point_of_interest = '0,0,1'
    self.effector_goal.center_point = '0,1,0'
  ...
  def test_init(self):
    self.assertEqual(type(self.client_instance.client), type(Client(move_linear_topic, MoveLinearAction)))
    # Following three must be in accordance to client_dict
    self.assertEqual(type(self.client_instance.goal), type(MoveLinearGoal()))
    self.assertEqual(self.client_instance.topic, move_linear_topic)
    self.assertEqual(type(self.client_instance.action), type(MoveLinearAction))
  ...
  def test_has_succeeded(self):
    self.linear_pub.publish(String('success:1'))
    rospy.sleep(1)
    self.client_instance.wait_server()
    self.client_instance.fill_goal(self.effector_goal)
    self.client_instance.send_goal()
  ...
  def test_has_aborted(self):
    self.linear_pub.publish(String('abort:1'))
    rospy.sleep(1)
    self.client_instance.wait_server()
    self.client_instance.fill_goal(self.effector_goal)
    self.client_instance.send_goal()
  ...
  def test_has_preempted(self):
    self.linear_pub.publish(String('preempt:1'))
    rospy.sleep(1)
    self.client_instance.wait_server()
    self.client_instance.fill_goal(self.effector_goal)
    self.client_instance.send_goal()
